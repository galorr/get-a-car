import { Component, inject, signal, computed, input, output, effect, model } from '@angular/core';
...
export class SideNavComponent {
  ...
  activeSection = signal<'registration' | 'alerts' | 'settings'>('registration');
  ...
  constructor() {
    ...
    // Use effect to handle form value changes for validation
    effect(() => {
      this.validateForm();
    });
  ...
  private validateForm(): void {
    ...
    if (form.get('preferredCarType')?.invalid && form.get('preferredCarType')?.touched) {
      errors.preferredCarType = 'Preferred car type is required';
    }

    if (form.get('rentalDuration')?.invalid && form.get('rentalDuration')?.touched) {
